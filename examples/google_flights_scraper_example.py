...
def get_time(soup_element):
    spans = (
        soup_element.find("div", "Ir0Voe")
        .find("div", "zxVSec", recursive=False)
        .find_all("span", "eoY5cb")
    )
...
def extract_flight_information_from_soup(soup_of_the_whole_page):
    ...
    for listing in flight_listings:
        if listing is not None:
            ...
            flight = {"airline": airline, "time": time, "price": price}
...
def extract_flights_data_from_urls(urls):
    ...
    for url in urls:
        ...
        constructed_flight_results.append({"url": url, "flight_data": flights})
